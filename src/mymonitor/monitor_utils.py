...
def run_and_monitor_build(
    ...
    output_summary_log_filename = (
        f"{base_filename_part}_summary.log"  # New summary log file name
    )

    output_csv_file = log_dir / output_csv_filename
    output_summary_log_file = (
        log_dir / output_summary_log_filename
    )  # Path for the new summary log
    ...
    if collector_type == "rss_pidstat":
        summary_log_header_content.append(
            f"Pidstat Aux Log (temporary, will be merged): {collector_aux_log_file.name}"
        )
    summary_log_header_content.append("-" * 80)
    ...
    collector_kwargs = {}
    if collector_type == "rss_pidstat":
        collector_kwargs["pidstat_stderr_file"] = (
            collector_aux_log_file  # pidstat still writes to its aux file first
        )
    ...
    def _run_and_log_command_to_summary(
        ...
        if exit_code != 0:
            logger.warning(
                f"{cmd_desc} command failed (code {exit_code}). Continuing..."
            )
        else:
    ...
    if setup_command_template:
        ...
        ):  # Handle 'source' for shell environment setup
    ...
    if clean_command_template:
        _run_and_log_command_to_summary(
            clean_command_template, "Clean", project_dir, True, output_summary_log_file
        )
    ...
    # 在采样循环外
    category_peak_sum = {}  # 每类别的总和峰值
    category_peak_single = {}  # 每类别单进程峰值
    category_pid_set = {}  # 每类别不同进程集合
    ...
    try:
        ...
        with open(output_csv_file, "a") as f_csv:
            for samples_at_interval in active_memory_collector.read_samples():
                ...
                category_mem_sum = (
                    {}
                )  # Sum of primary metric for each category in this interval
                all_mem_sum = (
                    0  # Sum of primary metric for ALL processes in this interval
                )
                category_pids_interval = {}  # PIDs for each category in this interval

                # First, write individual process samples
                for sample in samples_at_interval:
                    ...
                    metric_value_for_sum = 0  # For summing up primary metric
                    ...
                    for metric_name in metric_fields_header:
                        ...
                        if metric_name == primary_metric_to_track:
                            try:
                                ...
                                logger.warning(
                                    f"Could not convert primary metric '{metric_name}' value '{metric_val}' to int for sample: {sample.pid} {sample.command_name}"
                                )
                                metric_value_for_sum = (
                                    0  # Default to 0 if conversion fails
                                )

                    escaped_full_command = sample.full_command.replace('"', '""')
                    row_data_individual.extend(
                        [sample.pid, sample.command_name, f'"{escaped_full_command}"']
                    )
                    f_csv.write(",".join(row_data_individual) + "\n")

                    # Accumulate sums for this interval
                    all_mem_sum += metric_value_for_sum
                    category_mem_sum.setdefault(category, 0)
                    category_mem_sum[category] += metric_value_for_sum

                    # Collect PIDs for this interval (for unique PID counting later)
                    category_pids_interval.setdefault(category, set()).add(sample.pid)

                    # Update peak single process metric per category (already in your code, keeping it)
                    if primary_metric_to_track:
                        # This 'category_stats' is for single process peak, not category sum peak
                        if (
                ...
                # Update overall peak memory sum
                if primary_metric_to_track and all_mem_sum > peak_overall_memory_kb:
                    peak_overall_memory_kb = all_mem_sum
                    peak_overall_memory_epoch = current_epoch

                # Write CATEGORY_SUM and ALL_SUM rows to CSV
                # These rows have a different structure, plotter will need to be aware or filter them
                # Column structure for these: Timestamp_epoch, TYPE_MARKER, Category_Name_Or_ALL, Value_KB
                for cat, mem_sum_val in category_mem_sum.items():
                    f_csv.write(f"{current_epoch},CATEGORY_SUM,{cat},{mem_sum_val}\n")
                if (
                    primary_metric_to_track
                ):  # Only write ALL_SUM if we are tracking a primary metric
                    f_csv.write(f"{current_epoch},ALL_SUM,ALL,{all_mem_sum}\n")

                # Update peak sum for each category over the entire build
                for cat, current_cat_sum in category_mem_sum.items():
                ...
                if current_build_proc and current_build_proc.poll() is not None:
                    logger.info(
                        "Build process finished. Stopping memory collection for this build."
                    )
                    break
        ...
        if active_memory_collector:
            logger.info(
                f"Stopping memory collector ({active_memory_collector.__class__.__name__})..."
            )
        ...
        if current_build_proc and current_build_proc.poll() is None:
            logger.warning(
                f"Build process {current_build_proc.pid} still running in finally block, attempting to terminate."
            )
        ...
        duration_seconds = (
            int(end_time_seconds - start_time_seconds)
        ...
        if primary_metric_to_track:
            ...
            final_summary_log_lines.append(
                f"Peak Overall Memory ({primary_metric_to_track}): {peak_overall_memory_kb} KB (at {peak_time_str})"
            )
        else:
            final_summary_log_lines.append(
                f"Peak Overall Memory: N/A (no primary metric identified for sum)"
            )

        final_summary_log_lines.append(
            f"Peak Single Process Memory Usage (by category, based on {primary_metric_to_track if primary_metric_to_track else 'N/A'}):"
        )
        if category_stats:
        ...
        # Add Peak Category Sum Memory Usage
        final_summary_log_lines.append(
            f"Peak Category Sum Memory Usage (based on {primary_metric_to_track if primary_metric_to_track else 'N/A'}):"
        )
        ...
        # Add Unique Process Counts per Category
            ...
            if (
                ...
                try:
                    ...
                    try:  # Attempt to delete the aux file after merging
                        ...
                        logger.info(
                            f"Merged and deleted pidstat aux log: {collector_aux_log_file.name}"
                        )
                    except OSError as e_del:
                        logger.warning(
                            f"Could not delete pidstat aux log {collector_aux_log_file.name}: {e_del}"
                        )
