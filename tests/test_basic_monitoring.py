...
# Define the path to the fake build script used in tests.
# This makes it easy to reference and set permissions.
...
def setup_test_config_files(tmp_path: Path) -> Path:
    ...
    This fixture creates a full set of configuration files (`config.toml`,
    `projects.toml`, `rules.toml`) in a temporary directory provided by
    pytest's `tmp_path` fixture. This ensures that tests run in an isolated
    environment with a known, controlled configuration, preventing interference
    from any real configuration files on the system.

    The created configuration points all output to a `logs_output` directory
    within the temporary structure, making it easy for tests to find and
    validate the artifacts generated by the application.

    Args:
        tmp_path: The pytest fixture that provides a temporary directory path.
